...
def load_txt_documents(dataset_dir: Path) -> List[Dict[str, Any]]:
    """
    Load toàn bộ file .txt thành danh sách document nguyên vẹn.

    Mỗi file → 1 dict: ``{"doc_id", "path", "content", "metadata"}``.
    Không chia passage — chunking.py sẽ xử lý toàn bộ cấu trúc.
    """
